...
from hume_embedding import getEmbeddingsLanguage, combineFullDataFromJournal
...
def get_recording():
    ...
    # processedEmbeddings = getEmbeddingsLanguage("local.mp4")
    # journalInfo = {
    #     "entry_id": id,
    #     "time": time,
    #     "date": date,
    ...
    #     "type": "video",
    # }
    # fullData = combineFullDataFromJournal(journalInfo, processedEmbeddings)
    # print(fullData)
    # Save fullData as a json
    # with open(f"test/{id}.json", "w") as outfile:
...
def process_hume_results():
    ...
    sentences_embeds = embed_transcript_upload_pinecone(
        sentences, user_id=entry_data["user_id"], entry_id=entry_data["entry_id"]
    )
